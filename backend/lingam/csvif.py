...
import sys
import time
import fastvarlingam

def calculate_summary_matrix(data: np.ndarray, max_lags: int, measure: str) -> np.ndarray:
    """
    ...
    Args:
        ...
        measure: LiNGAM measure to use (pwling or pwling_v2)
        
    Returns:
        summary_matrix: Matrix of causal relationships
    """
    model = fastvarlingam.VARLiNGAM(lags=max_lags, prune=False, lingam_measure=measure)
    model.fit(data)
    print('Causal order:', model._causal_order)
...
def process_csv(input_file, output_file, time_file, weighted_edgelist_file, max_lags, weight_threshold, measure):
    # Read data
    if input_file == 'stdin':
    ...
    summary_matrix = calculate_summary_matrix(data, max_lags, measure)
...
def main():
    ...
    process_csv(args.input, args.output, args.time, args.weighted_edgelist, 
